...
const moduleText=String.raw`#!name=YouTube Reviewed
#!desc=公开仓库审查更新版。无需令牌；启用本机证书即可使用。实际去广告效果需实机验证。
...
`;
fs.writeFileSync(dir+'/YouTube-Reviewed.sgmodule',moduleText);
fs.writeFileSync(dir+'/YouTube-Reviewed-Private.sgmodule',moduleText); // Legacy URL, identical public module

const manifest={version:'1.0.1',repositoryVisibility:'public',date:'2026-09-21',upstreamRepo:'Maasea/sgmodule',upstreamCommit:'65075cdb388fc5e3094afd7e7314c67b243f3525',upstreamResponseBlob:'becad8eaa6094c189ea8db6d644de68ac2d66f61',upstreamResponseSha256:original,publishedResponseSha256:crypto.createHash('sha256').update(s).digest('hex'),status:'reviewed-with-restrictions-device-validation-pending',excluded:['youtube.request.js','third-party init-stream.maasea.workers.dev relay','config/log_event capture','playback unlocking','translation','guide/settings modifications'],publicRepositoryApiVerified:true,deviceValidation:{automaticRefresh:false,adBlocking:false}};
